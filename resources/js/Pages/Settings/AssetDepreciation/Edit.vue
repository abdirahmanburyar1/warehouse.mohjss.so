<template>
    <AuthenticatedLayout :title="'Edit Asset Depreciation Setting'" description="Modify depreciation configuration">
        <div class="max-w-4xl mx-auto">
            <!-- Header -->
            <div class="mb-6">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-2xl font-semibold text-gray-900">Edit Asset Depreciation Setting</h1>
                        <p class="text-gray-600 mt-1">Modify existing depreciation configuration</p>
                    </div>
                    <div class="flex space-x-3">
                        <Link 
                            :href="route('settings.asset-depreciation.show', { asset_depreciation: setting.id })"
...
const submitForm = () => {
    ...
    router.put(route('settings.asset-depreciation.update', { asset_depreciation: props.setting.id }), form.value, {
        onFinish: () => {
            isSubmitting.value = false;
        }
    });
...
onMounted(() => {
    form.value = {
        ...
        metadata: props.setting.metadata || {},
    };
    
    if (props.setting.type === 'json') {
        jsonValue.value = JSON.stringify(props.setting.value, null, 2);
    }
});
